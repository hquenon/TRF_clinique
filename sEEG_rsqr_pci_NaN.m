...
gain  =  sorciere_pred.r2_real*100 - sorciere_acou.r2*100 ;
gain(pvals >= 0.05) = 0;  % Supprimer les non-statistiques
predictive_gain = max(gain, 0);


% % Plot TRF result
% Masquer les canaux non significatifs
pval_thresh = 0.05;
mask = sorciere_pred.pval < pval_thresh;
predictive_gain_masked = predictive_gain;
predictive_gain_masked(~mask) = NaN;

trf_p                     = [];
trf_p.powspctrm(:,1,:)    =  repmat(predictive_gain_masked,2,1)';
trf_p.powspctrm(:,2,:)    =  repmat(predictive_gain_masked,2,1)';
...
gain = pianos_pred.r2_real*100 - pianos_acou.r2*100;
gain(pvals >= 0.05) = 0;  % Supprimer les non-statistiques
predictive_gain = max(gain, 0);
...
% Masquer les canaux non significatifs
pval_thresh = 0.05;
mask = pianos_pred.pval < pval_thresh;
predictive_gain_masked = predictive_gain;
predictive_gain_masked(~mask) = NaN;
